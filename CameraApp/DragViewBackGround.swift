...
class TinderViewBackGround: UIView {
    ...
    var currentIndex = 0
    var currentLoadedCardsArray = [TinderCard]()//放置已经加载完毕的卡片
    ...
    func setUpView() {
        backgroundColor = UIColor(red: 46/255, green: 41/255, blue: 61/255, alpha: 1)
        valueArray = ["first", "second", "third", "fourth", "last"]
        currentIndex = 0
        loadCards()
    }
    ...
    func loadCards() {
        if valueArray.count > 0 {
            ...
            for (i,_) in currentLoadedCardsArray.enumerated() {
                if i > 0 {
                    insertSubview(currentLoadedCardsArray[i], belowSubview: currentLoadedCardsArray[i - 1])//A insertSubView B belowSubview:C  是将B插入A并且在A已有的子视图C的下面
                }
                else {
